...
| Settings in the folder the installer owns | The app keeps its files in `%AppData%\AorusControl`, not `%LocalAppData%\AorusControl`. Sharing that folder meant merely *running* the app from a build tree created it, and the installer then refused to install on a machine that had never installed anything - and an uninstall would have taken the saved fan curve with it. |
| Guessing at what a vendor preset does | The fan profiles publish no curve, so nothing is invented for them: the app quotes the one curve Gigabyte's own software has for this model, from its decompiled source, and says where it had to be adapted to this firmware's limits. |
